...
\multirow{2}{*}{Database Name} &
  ...
  Src &
   ...
   \\
\hline
\begin{tabular}[c]{@{}l@{}} Dokmani\'c \textit{et al.} \cite{Dokmanic2013acoustic}\end{tabular} &
  \cmark  &
  $\sim$ &
  \multicolumn{1}{c|}{$\sim$} &
  \multicolumn{1}{l|}{15} &
  ...
  Non shoebox room &
  RooGE \\ \hline
\begin{tabular}[c]{@{}l@{}} Crocco \textit{et al.} \cite{Crocco2017uncalibrated}\end{tabular} &
  \cmark &
  $\sim$ &
 ...
 Remaggi \textit{et al.} \cite{remaggi2017acoustic} &
  \cmark & % # noete pos?
  $\sim$  & % # note echo?
  \cmark & % # note room?
  ...
  \\
\hdashline
Remaggi \textit{et al.} \cite{Remaggi2019modeling} &
  \cmark & % # noete pos?
  $\sim$  & % # note echo?
  \cmark & % # note room?
  ...
  \\
\hline
BUT Reverb dB \cite{Szoke2018building} &
  \cmark &
  \xmark&
  $\sim$ &
  ...
  SE/ASR \\ \hline
VoiceHome \cite{Bertin2019voice} &
   \cmark &
   \xmark&
   \xmark&
  \multicolumn{1}{l|}{188} &
...
The read is invited to refer to \cite{Szoke2018building, Genovese2019blind} for more complete list of existing RIR datasets.
%\protect\\- indicates minimum and maximum number of variable number of objects
\protect\\$^{\dagger}$The dataset in \cite{remaggi2017acoustic} is originally intended for RooGE and further extended for (binaural) SE in \cite{Remaggi2019modeling} with a similar setup.}
\end{table*}