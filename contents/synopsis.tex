\newcommand{\synopsisChAcoustics}{
    ...
    By assuming some approximations, \acp{RIR} will be described by parts in relation to methods to compute them.
...
\newcommand{\synopsisChLantern}{
    In this chapter, we use virtually supervised deep learning models to learn the mapping from microphone recordings to the echoes' timings.
...
\newcommand{\synopsisChApplication}{
    In this chapter, we present a selection of algorithms and methodologies which we identified as potential beneficiaries of echo-aware additions.
    At first, we present a typical scenario that highlights some cardinal problems.
    Then, state-of-the-art approaches to address these problems are listed and commented in dedicated sections, highlighting the relationship with some acoustic propagation models, respectively.
...
\newcommand{\synopsisChSeparake}{
    In this chapter, echoes are used for improving performance of classical \acf{MASS} methods.
    Existing methods typically either ignore the acoustic propagation or attempt to estimate it fully.
    Instead, this work investigates whether sound separation can benefit from the knowledge of early acoustic echoes.
    These echo properties are derived from the known locations of a few \textit{image microphones}.
    The improvements are shown for two variants of a method based on non-negative matrix factorization based on the work of~\citeonly{ozerov2010multichannel}: one that uses only magnitudes of the transfer functions and one that uses the phases as well.
