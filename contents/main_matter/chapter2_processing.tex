...
An alternative way is to consider relative filters between channels for a given source.
Given a pair of microphone $i$ and $i'$, their noisy observation writes
\begin{align}
    c_i = s \conv h_i + u_i
    c_i' = s \conv h_i' + u_i'
\end{align}

This representation allows to resolve gain ambiguities and it comes particularly useful since its estimation is ``easier'' than
the esti.
