...
\subsection{The DFT as approximation of the FT}
...
Consciously of the above approximations, we can now rewrite our signal model for the discrete case.
...
where $\convDis$ is the finite-time linear convolution operator\sidenote{
    ...
    .
}. The filter is the room transfer function for which we can use the closed-form echo model~\cref{eq:acoustics:ims:freq} derived in the previous chapter:
\begin{equation}\label{eq:processing:discreteModel:rir}
    \hat{H}_{ij}[ k] = \sum_{\idxEch=0}^{\numEchs}
...
\mynewline
Although used in practice, this model makes use of some approximations that are worth presenting.
In particular the work by~\citeonly{tukuljac2018mulan} properly discuss them in the context of modeling the echo estimation problem.
The issues arise from our willing of simultaneously using the closed-form \RIR/ model and the sampled observations $\hatx_i[n]$.
...
The diagram shows a chain of operators (sampling and transforms) with provable and approximated equivalences that lead to~\cref{eq:processing:discreteModel}. In order,
\begin{enumerate}[label=(\roman*)]
    ...
    Thus, the first approximation~\ref{it:processing:dft:approx1} considers $\tilde{h}[n] \approx (\lowpassfilter \convDis \tilde{h})[n]$, in words
