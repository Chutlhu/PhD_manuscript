...
In order to better formalized it, in source separation (\eg/ \citeonly{sturmel2012linear,gannot2017consolidated}) it is common to use an intermediate representation called \emph{source spatial images}:
...
In case of uniformly spaced samples at rate $\Fs$, it produces a function of continuous frequency that is a periodic summation of the continuous Fourier transform of the original continuous function.
...
\begin{equation}\label{eq:processing:discreteModel:rir}
    ...
    .
\end{equation}
Note that, the natural frequency $f_k$ in $\si{\Hz}$ corresponding to the $k$-th frequency bin can be computed as
...
Moreover although the real-world \acp{RIR} $h_1$ and $h_i$ are causal, their \ReTF/ needs not be so.
...
\begin{itemize}
    ...
    \item the problem of estimating the \ReTF/ can be considered ``easier'' than \acp{RIR} estimation;
    ...
    \item the \ReTF/ encodes acoustics properties of the related \acp{RIR} and is independent on the source signal.
