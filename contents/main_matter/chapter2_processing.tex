...
\newthought{The distrete \STFTdef/}\marginpar{\footnotesize%
...
many algorithm are designed directly in the time domain or alternatives \TF/ representation\sidenote{such as MelScale, FilterBank, learned-TF reperestation}.
...
In the time-frequency domain, $\imgs_j(l,k)$  and $\src_j(l,k)$ are the \STFT/ of $\imgs_j(t)$ and $\src_j(t)$ respectively.
...
\end{equation}
where $\bsu(n,k) = \bsn(n,k) + \boldsymbol{\varepsilon}(n,k)$ is the contribution of all the diffuse noise sources and modeling error.
...
Dropping the dependency on $j$ for readability and taking the $i$-th as a reference, the \RTFdef/ associated the the $i'$-th channel is defined as~\citeonly{gannot2001signal}
\begin{equation}\label{eq:processing:rtf}
    \rtf_{i'}[k] = \frac{\rir_{i'}[k]}{\rir_i[k]}
    ,
\end{equation}
...
Considering the noisy observation $x_i$ and $x_{i'}$, their signals can be re-written in term of $\rtf_{i'}$ as follows
\begin{equation}
    \begin{matrix}
    \begin{aligned}
            \mic_i &= \rir_i \conv s + \allNoise_i, \\
            \mic_{i'} &= \rir_{i'} \conv s + \allNoise_{i'},
    \end{aligned} & \kto  & \begin{aligned}
        \mic_i &= \rir_i \conv s + \allNoise_i, \\
        \mic_{i'} &= \rtf_{i'} \conv \rir_{i} \conv s + \allNoise_{i'}.
    \end{aligned}
    \end{matrix}
\end{equation}
Therefore, notice that $\rir_{i'} = \rtf_{i'} \conv \rir_{i}$, corresponding to~\cref{eq:processing:rtf} in the frequency domain.
Note that although real-world acoustic channels $h_i$ and $h_{i'}$ are causal, the $\rtf_{i'}$ need not be so.
