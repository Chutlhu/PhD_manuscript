...
\newthought{Signals are emitted by Sources} and are observed, received or recorded by microphones.
...
Some taxonomy through dichotomies:
...
\begin{center}
    ...
    contribution\sidenote{\eg/ the propagation from the point source to the microphone including reverberation}
...
\marginpar{%
        ...
        convolutive     & $x_{i} \ =\sum ^{J}_{j=1}( h_{ij} \ast s_{j})( t)$ \\
        \bottomrule
        \end{tabular}
        \captionof{table}{Taxonomy of linear mixing models for a mixture channel $x_i$, sources $s_j$, filter response $h_{ij}$,
...
\begin{align}\label{eq:processing:mixing}
...
\subsection{Target, Interferer and Noise}
\openepigraph{%
...
\marginpar{
    \centering
        ...
        \captionof{figure}{Graphical representation of the mixing model~\ref{eq:processing:mixing}.}\label{fig:processing:mixing}
}
...
\newthought{Measurement or Model Noise} account for general residual miss- and under-modeling error.
As common is signal processing and information theory, this error term will be modeled as \AWGN/.

\section{Spectral representations}\label{sec:processing:domains}
