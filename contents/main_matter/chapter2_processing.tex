...
Today signals are typically processed, stored and analyzed by computers and modules as \textit{digital audio signal}.
This corresponds to the discrete-time signal $\hat{x}$ obtained by periodically sampling\sidenote{%
...
} the continuous-time signal $\tilde{x}$ at rate $\Fs\;[\si{\Hz}]$.
...
% Note also the use of square brackets and $\hat{}$ in the notation for a discrete-time signal $\hatx[n]$,
% which avoids ambiguity with the notation $\tilde{x}(t)$ used for a continuous-time signal.
...
As explained in~\citeonly{tukuljac2018mulan}, issues arise from simultaneously using the closed-form \RIR/ model derived in~\cref{eq:processing:rir:ft}
