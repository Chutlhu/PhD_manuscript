...
Instead, we can keep the infinite summation by multiplying the sampled signal by a discrete-time window function $\hat{w}$ selecting the non-zero porting of $x$, $x_N[n] = \hat{w}[n]x[n]$.
...
\begin{equation}\label{eq:processing:discreteModel}
    \hat{\img}_{\idxMicSrc}[n] = (\hat{\rir}_{\idxMicSrc} \convDis \hat{\src})[n]
    \mathspace\overset{\discreteFT{}}{\kto}\mathspace
    \hat{C}_{\idxMicSrc}[k] = \hat{H}_{\idxMicSrc}[k] \hat{S}[k]
    ,
\end{equation}
...
\\The filter $\hat{H}_{\idxMicSrc}[k]$ is the \DFT/ of the room impulse response.
...
\begin{equation}\label{eq:processing:discreteModel:rir}
    \hat{H}_{\idxMicSrc}[ k] \approx \sum_{\idxEch=0}^{\numEchs}
...
\begin{enumerate}[label=(\roman*)]
    ...
    The third approximation regards the closed-form of $h_{\idxMicSrc}(f)$ of~\cref{eq:processing:discreteModel:rir} which
...
Equivalently, \cref{eq:processing:stft} can be expressed as \DFT/s of windowed frames, $X[k, l] = \discreteFT{x[n,l]}$ where $x[n,l] = x[n + l \hopsize] \hatw[n]$.
