...
Alternatively, approaches in~\citeonly{weinstein1994iterative, taghizadeh2015spatial, salvati2016sound} attempt to solve \ac{SSL} by estimating the full \acp{RIR} (or \acp{ReTF}~\citeonly{li2016estimation}), which is known to be a very difficult task.
...
The authors of \citeonly{nakashima2010localization} study the \ac{SSL} problem in binaural recordings.
...
Note that answering this question is an \textit{impossible} task in free- and far-field stereophonic conditions, due to the well known spatial ambiguity of a single \ac{TDOA}, referred to as the \textit{cone of confusion}~\citeonly{bregman1990auditory}.\marginpar{%
\centering
\footnotesize
\includegraphics[width=\linewidth]{mirage/cone.png}
\captionof{figure}{%
    Illustration of the cone of confusion.}
\label{fig:mirage:cone}
}, that is a cone-shaped region where a source provide the same \TDOA/ and (see~\cref{fig:mirage:cone}).
...
\begin{figure}
    ...
    \end{sidecaption}
\end{figure}
\\Then, \ac{SSL} reduces to estimating the \ac{TDOA}, which can be done by a \ac{CC}-based methods, \eg/, the \acf{GCC-PHAT} method\marginpar{
...
Despite the theoretical limitation of \ac{CC}-based methods presented in~\citeonly{chen2006time}, they are known to work well in practice under moderate reverberation and noise.
...
\footnotesize
    ...
    \captionof{figure}{%
        Illustration of the images $\mathring{m}_1$ and $\mathring{m}_2$ of microphones $m_1$ and $m_2$ in the presence of a reflective surface and a source.
...
In the first investigation of~\citeonly{di2019mirage}, we proposed to use Gaussians functions centered at those estimates with variances equal to the prediction errors made by the \ac{DNN} on the validation set.
...
However, here we are reporting the results obtained in the work~\citeonly{di2019mirage}, antecedent to the experimental results reported in~\cref{sec:lantern:robust}.
A free and open-source Matlab implementation of \ac{SRP-PHAT}\sidenote{
    \label{note:mbss}\href{http://bass-db.gforge.inria.fr/bss_locate/}{\library{MBSSLocate}~\ExternalLink}
} is used to aggregate local angular spectra obtained from the model output.\sidenote{
...
using physics-driven regularizers such as in~\citeonly{nabian2020physics} and making the learning phase independent of the array geometry.
