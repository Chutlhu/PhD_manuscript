...
The \RIRs/ in~\cref{eq:mirage:signalmodel} follows to the well known \acf{ISM}, where all the reflections are treated as mirror images of the true source with respect to reflective surfaces, emitting the same signal.
\marginpar{%
\centering
\footnotesize
...
We will employ here a less common but equivalent interpretation of \ISM/, namely, the image-microphone model (see~\cref{sec:separake:sota})
...
To this end, the 3 real-value estimates in $V$ are converted into \textit{synthetic} local angular spectra (see~\cref{subsec:mirage:2D-SSL}).
...
At first, we will consider the continuation of the investigation started in~\cref{sec:lantern:simple}, namely, the analysis is limited to the stereophonic recording in a single-source close-surface scenario.
...
A free and open-source Matlab implementation of \ac{SRP-PHAT}\sidenote{\label{note:mbss}\href{http://bass-db.gforge.inria.fr/bss_locate/}{\library{MBSSLocate}\ExternalLink}} is used to aggregate local angular spectra obtained from the model output.\sidenote{
    For SRP-PHAT we used a sphere sampling with $\ang{0.5}$ resolution and coordinates $\theta \in [-179, 180]$ and $\phi \in [0, 90]$ is used for the \c{DOA} search.
...
Unfortunately external noise is decrease the accuracy dramatically and suggest that we should consider corrupting noise in the training as well as more complex \ac{DNN} architecture.
Nevertheless, it is interesting to notice that our model trained and validated with white noise sources somewhat generalizes to speech sources.

\mynewline
With this work, we demonstrated how a simple echo model could allow 2D SSL with only two microphones, using simulated data.
In the following section, we will evaluate the \ac{MIRAGE} framework on multichannel simulated data inspired a real world application.
