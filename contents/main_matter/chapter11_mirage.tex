...
\subsection{Synthetic-data Multi-channel scenario}
...
\mynewline
In~\cref{fig:mirage:tdoa_synth_snr} the \TDOA/ estimation error is illustrated against the \SNR/ level of the recordings.
When the source signal is noise, both methods yield similar mean error independently of the noise level.
However, \ac{GCC-PHAT} seems to produce more outliers lower \SNR/.
...
\subsection{Real Multichannel Scenario}
...
The HARU was placed on top of a table with a height of 0.10 m to simulate the close-reflector scenario, used to train the DNN model.
...
This can be seen also in~\cref{fig:mirage:tdoa_real_signals}.
...
\section{Conclusion}
In this chapter, we demonstrated yet anther application of acoustic echoes.
We addressed on \ac{SSL} problem and we showed how a simple echo model could boost performance in strongly echoic scenarios, that is, when microphones are placed close to a reflector.
