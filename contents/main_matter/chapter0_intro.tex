...
Everything you are hearing now, is what is called the \textit{audio scene}\sidenote{
    The more correct terminology for it is \textit{auditory scene analysis}, which relates to the human perception.
    It was coined by psychologist Albert Bregman in~\citeonly{bregman1990auditory}.
    However, as we extend this concept to audio signal processing and as it is commonly used in the literature, we will use this terminology.
}.
...
\cref{tab:processing:problem} lists some selected audio scene analysis problems that will be considered later in this thesis.
...
\begin{itemize}
    \item  \textit{What?} Answered by Audio Source Separation, Speech2Text and Source Identification, by operating on the semantic content of the source signals.
...
\mynewline
And it is in this perspective that echoes help.


\section{Echo-aware approach}
As is proven by animal behaviors, acoustic echoes are important proxy for understanding the audio scene analysis.
