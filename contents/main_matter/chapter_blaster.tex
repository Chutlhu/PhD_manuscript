...
This chapter recall the main findings of the paper bringing additional insight in the existing and proposed models for \AER/.
...
\newthought{The resulting algorithm} used to solve~\cref{eq:blaster:TV-BLASSO} is an instance the sliding Frank-Wolfe algorithm proposed in~\citeonly{denoyelle2019sliding} to solve~\cref{eq:blaster:TV-BLASSO}.
...
\begin{itemize}
    \item \textit{Anchor or previously found peaks}
    \item \textit{\textit{Local} cost based on Cross-relation}
    \item \textit{Find the maximizer}
    \item \textit{Update the amplitudes (Lasso-like)}
