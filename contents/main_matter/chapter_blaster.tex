\chapter{Knowledge-driven Acoustic Echo Retrieval \& \library{Blaster}}\label{ch:blaster}
...
Some existing approaches methods rely on a known source signal~\citeonly{park2017compressive,jensen2019method}.
...
\mynewline
Of particular interest the proposed approach is the recently proposed framework of \CDdef/~\citeonly{candes2014towards}.
...
\section{Signal and measurement model}
...
\begin{itemize}
    ...
    \item $\scrT(\disRecordedSignal_i)$ is the $(2N+L-1) \times L$ Toeplitz matrix (build as shown is~\cref{fig:blaster:toeplitz})\marginpar{
        \centering
        \resizebox{\linewidth}{!}{
...
Before, let us show how to express to express the on-grid \BCE/ problem proposed by~\citeonly{lin2008blind} (See \cref{tab:blaster:sota}) as a standard \LASSO/ problem.
...
\newthoughtpar{From LASSO to BLASSO}
...
The proposed method (\BLASTER/) is compared against the non-negative $\ell_1$-norm method (\algoBsn) of~\citeonly{lin2007blind} and the iterative $\ell_1$-norm approach (\algoCrocco) described in~\citeonly{crocco2016estimation}.\sidenote{
    A reference implementation for \algoCrocco{} and \algoBsn{} were provided by the authors of~\citeonly{crocco2016estimation}.
}
...
\begin{table}[ht]
    \begin{sidecaption}[]{
        Summary of the dataset used for evaluation. \epsdice[black]{3} and \epsdice{5} stands for randomly sampled from a continuous and discrete set of values respectively with uniform low.
...
This is promising since the practical advantage of knowing the timing of two echoes per channel has been demonstrated in~\citeonly{di2019mirage} (See~\cref{ch:mirage}), and in~\citeonly{scheibler2018separake} (See~\cref{ch:separake}).
