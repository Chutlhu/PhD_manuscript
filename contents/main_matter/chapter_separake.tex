...
The approach proposed is based on the acoustic rake receivers proposed in~\citeonly{Dokmanic:2015dr} and thus dubbed as \SEPARAKEdef/.
...
\newthought{Traslating echoes into image arrays} provides a interesting geometrical interpretation in light of beamforming theory.
Real and virtual microphones form dipoles with diverse frequency-dependent directivity patterns.
By integrating more and more virtual microphones the directivity pattern, \aka/ beampatterns, changed and higher spatial selectivity can be achieved~\citeonly{dokmanic2015raking}.
...
\newthought{The MU cost function} is minimize the \textit{Itakura-Saito divergence} \citeonly{Fevotte:2011af} between the observed spectrogram $(V_{\idxMic}[k,l]$ and the model $\wh{V}_{\idxMic}[k,l]$, that is,
\begin{equation}
...
\newthought{The \EM/ algorithm} alternate between the estimation of the parameter (E-step) and the maximization of expectation of the log-likelihood given in~\cref{eq:separake:loglike}.
