...
\gccmt{perche' a wave? l'hai nominata prima: direi tipo che il sound propagates through a medium. e poi wave l'hai gia' messo in corsivo prima}
\gccor{A \textit{wave} is a disturbance that}{Thus sound} propagates though a medium, which can be solid, liquid or gaseous.
...
% In general, the sound field is complex and can be decomposed as a superimposition of several waves.
We think of this process in the light of the classic \textit{source-medium-receiver} model of communication theory.
The \textit{source} is anything that emits or expends energy (waves)\gccmt{non mi piace che dici energy, di direttamente wave cosi' sei corente con i vocaboli e non aggiungi termini nuovi: non hai mai parlato di energia}\sidenote{%
...
% The rest of this section introduce of this equation, re-arranging the derivation presented in \citeonly{kuttruff2016room, }.
...
as a function of the position $\positionMicrophone$ $[\si{\metre}]$ and time $t$ $[\si{\second}]$ \gccmt{non mi piace che metti le unita' di misura}
\begin{equation}
...
\begin{itemize}
    \item the \textit{conservation of momentum}\sidenote{In fluidodynamics, it comes with the name of the Euler's equation. \gccmt{inutile}},
    \item the \textit{conservation of mass}, and
    \item the \textit{polytropic process relation}\sidenote{meaning that the medium is an ideal gas undergoing a reversible adiabatic process. \gccmt{questa la metterei nel testo: non  e' quella che ti  dice che il gas deve essere omogeneo? alloara l'however dopo avrbbe senso}}.
\end{itemize}
\gccmt{howver a cosa?}
...
The\gcrm{ wave} equation~\ref{eq:acoustics:wave} is expressed in the space-time domain $\depSpaceTime$.
...
\begin{equation}
    ...
    ,
\end{equation}
where $k = \frac{2 \pi f}{c}$  is known as \textit{wave number} $[\si{\metre^{-1}}]$, \gccor{that}{and} relates the frequency $f \; [\si{\hertz}]$ \gccor{and}{to} the propagation velocity $c$.

Both the wave\gcrm{ equation}~\ref{eq:acoustics:wave} and the Helmholtz's equation~\ref{eq:acoustics:helmholtz} are source-independent,
namely no source is present in the medium.
Therefore they are \gccor{called}{said to be} \textit{homogeneous} as the right-hand term is zero.
\gccmt{Non andrei a capo}
...
In the presence of a sound source producing waves with distribution function $s(t, \positionMicrophone)$, the wave equation can be written\marginpar{%
    \footnotesize
    The minus sign is a conventions. \gccmt{inutile}
}
\begin{equation}
...
\begin{equation}
    ...
    ,
\end{equation}
The function $H(f, \positionMicrophone \mid \positionSource)$ that satisfy \cref{eq:acoustics:green_definition} is \gc{called} the \textit{Green's function} \gc{and is}
...
\begin{quote}
    ...
    in signal processing.\gccmt{citazione! analogo dove? in fisica? in acustica}
    }
\end{quote}
Under this light, the physic so-far can be rewritten \gccor{in}{using} the vocabulary of the communication theory, namely \textit{input}, \textit{filter} and \textit{output}.

According to Green's method, the equations above can be solved in the frequency domain for arbitrary source as follows:
\marginpar{%
    \footnotesize
    If one ignores the space integral, one can see the close relation with a transfer function. \gccmt{nel testo}
}
\begin{equation}
...
\RIRs/ usually exhibit common structures.
...
is the (cumulative) frequency-independent absorption coefficient of the surfaces between $\positionSource_\idxEch$ and $\positionMicrophone$.
...
\mynewline
Before properly defining $\bar{\absCoeff}_{\idxEch}$, the reader should notice that the model~\eqref{eq:acoustics:ims:time} induces an ``order'' among reflections indexed by $\idxEch$.
Reflections can be ordered based on their \TOA/, $\tau_r = \norm{\positionMicrophone - \positionSource_\idxEch}/\speedOfSound$,
or their amplitudes, $\bar{\absCoeff}_{\idxEch}/(4 \pi \norm{\positionMicrophone - \positionSource_\idxEch})$.
Moreover, one can sort them according to their generation, \eg/ direct path, first-order images \etc/.
However, this requires an arbitrary order within the same generation, typically a fixed wall sequence.
Notice that the resulting sorted sequences can differ substantially as show in~\cref{fig:acoustics:rir:ref_label}
\\Assuming a shoebox room and a generation-based ordering, $r = 0$ refers to the ideal propagation path, hence, $\bar{\absCoeff}_0 = \absCoeff_0 = 1$.
$r \in \kbrace{1, \ldots, 6}$ refers to the first-image sources which separated from the receiver by a single and real plane;
therefore $\bar{\absCoeff}_r$ coincides with the wall reflection coefficient.
For $r \geq 7$, the ray from $\positionSource_r$ to $\positionMicrophone$ intersects more the two surfaces.

\mynewline
The above equation assumes perfectly rigid and reflective walls.
...
of real surfaces, the Fourier transform of~\cref{eq:acoustics:ims:time} is considered instead, where
...
Notice that now the damping coefficient acts as a filter,
requiring~\cref{eq:acoustics:ims:time} to be written as sum of convolutions.
...
\questionpar{Can echoes be louder the the direct-path?} Yes.
