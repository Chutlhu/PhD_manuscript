...
\begin{itemize}
    ...
    \item the \textit{equation of state} (\aka/ the gas equation)
\end{itemize}
...
\newthought{Derivation:}
Consider the infinitesimal volume unit $\volumeUnit$ of a fluid or gas (such as air), whose center of gravity is located at $\positionMicrophone$.
Let $\mass$ be the mass of the volume. By the well-known Newton's second law, applying a force $F$ to the fluid, its acceleration increase proportionally, namely:

\begin{equation}
    F = \mass \kpderiv[]{\bfv(\positionMicrophone, t)}{t}
\end{equation}
where $v(\positionSource, t)$ denotes the volume velocity and $t$ the time $[\si{\second}]$.
The force can be expressed in terms of difference of pressure $p$ on a surface of the volume, $V$, namely
\begin{equation}
    F = - \nabla \pressure(\positionMicrophone, t)
\end{equation}


\newthought{A solution to this} renowned equation can be found in many ways, depending on the number of dimensions, initial and boundary conditions.
The solution proposed utilize the Helmholtz's methods
...
\newthought{The Green's Functions} are tools used to solve linear differential equations with specified initial- and boundary- condition \cite{Duffy2015}.
They have been used to solve many fundamentals equation, among which \cref{eq:acoustics:helmholtz} and \cref{eq:acoustics:wave} for both free and indoor propagation.
\sidenote{%
...
Under this light and by assuming the fluid  medium as the \textit{linear filter}, the physic so-far can be rewritten in terms of input, filter and output
...
The function $H(f, \positionMicrophone \mid \positionSource)$ is the Green's function in frequency domain, or \textit{Transfer Function}, defined as the solution of the following inhomogeneous equation
\begin{equation}
    ...
    ,
\end{equation}
where we have assumed the source to be unit-amplitude harmonic point source at in $\positionSource$,
namely $S(\positionSource, f) = \delta(\positionMicrophone - \positionSource)$ representing an outward travelling wave.
\marginpar{%
The reader may appriciate here the parallelism with the impulse response, defined the response of a system to an impulse.
}
It can be shown \cite{Kutruff} that the time-invariant Green's function for \cref{eq:acoustics:helmholtz} writes
\begin{equation}
    G(f, \bfx \mid {\bfx}_0) = - \frac{1}{4 \pi \norm{\bfx - {\bfx}_0}} e^{- \frac{\Ii 2 \pi f \norm{\bfx - {\bfx}_0}}{\speedOfSound}}
\end{equation}
where $\norm{\cdot}$ denotes the Euclidean norm.
This function can be interpreted as the free-field transfer function between the source at x0 and the receiver at x.

The initial sound pressure $\pressure(\positionMicrophone, t)$ can now be computed by
taking the frequency-directional inverse Fourier transform of \cref{eq:acoustics:helmholz_conv}.
% Now taking advantage of the linear system theory we can obtain
% Combining the relations between the theory of linear systems and physical propagation of acoustic signals with small amplitude in the fluid medium,
