...
This chapter will build a first important bridge: from physics to analog signal processing.
...
It first defines sound and how it propagates~\cref{ch:acoustics:sec:wave} using the concept of impulse response.
Then the interaction with the environment is shown in~\cref{ch:acoustics:sec:reflection}, teasing out the fundamental concept of this thesis: the echoes.
...
Some auxiliary technical details appear in Appendix A.
...
Pushing layer by layer forward, a \textit{longitudinal wave}\sidenote{%
...
The changes in air pressure at one point in space can be represented by a \textit{waveform}, which is a graphical representation of a sound
...
\begin{description}
    \item[source] is anything that emits or expends energy (waves)\sidenote{%
...
This has two implications~\citeonly{kuttruff2016room}:
\begin{itemize}
...
In general media are not uniform and features inhomogeneities of two types:
