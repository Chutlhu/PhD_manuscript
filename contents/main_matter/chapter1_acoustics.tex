...
Even if the basic idea is rather inutile and simple, the model
is able to produce the exact solution to the wave equation for a 3D shoebox with rigid walls.
...
First specular ray tracing will eventually find all the same reflection paths as the IS technique although the volumetric receivers may cause some false detections
...
\marginpar{%
    ...
    \captionof{figure}{%
    Time-Frequency profile of reverberation Adapted from \citeonly{Badeau2019common, Wayverb}.
    }
...
The main drawback of this approach is it may leads to strong approximation error at low frequencies:
here the wavelength are of the same order of the wall surface and
interferences, resonances, diffractions effects are not taken into account at these frequencies\cite{Savioja2015goemetric}.
This
