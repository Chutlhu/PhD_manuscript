...
It first defines sound and how it propagates \gc{in the environment}~\cref{ch:acoustics:sec:wave} \gccor{using the concept of impulse response.}{introducing the fundamental concepts of echoes~\cref{ch:acoustics:sec:reflection} and \RIRdef/~\cref{ch:acoustics:sec:rir}}
\gcrm{\gccor{Then the interaction}{How the sound interacts} with the environment is shown in~\cref{ch:acoustics:sec:reflection}, teasing out the fundamental concept of this thesis: the echoes.
By assuming some approximation, the \RIRdef/ will be defined ~\cref{ch:acoustics:sec:rir} describing methods to compute \gcrm{or approximate }it.}
Finally in~\cref{ch:acoustics:sec:perception} \gccor{a description of the way}{how} the human auditory system perceives reverberation will be reported.
...
% The rest of the section is reproduce the derivation of this equation, re-arranging the derivation presented in \citeonly{kuttruff2016room, pierce2019acoustics, marczuk2006modelling, avanzini2019sound}.

% \marginpar{%
%     \small
%     \textbf{Sound}, noun, from Middle English sownde, alteration of sowne, borrowed from Anglo-Norman sun, soun, Old French son, from accusative of Latin sonus.
% }
\section{Sound Wave Propagation}\label{ch:acoustics:sec:wave}
According to common dictionaries and encyclopedias \gccmt{I would cite the exact source},
\begin{center}
    \textit{sound is the sensation perceived by the ear caused by the vibration of air}.
\end{center}
\gccor{Sound has then two aspects:}{This definition highlights two aspects of sound:} a physical one, characterized by the \gccor{vibrating air particles}{vibration of air particles}; and a \gccor{perceptive}{perceptual} one, involving the \gcrm{an} auditory system.
% \marginpar{
%     \small
...
% }
Focusing on the former phenomenon, when vibrating objects excites air, \gc{surrounding} air molecules starts oscillating,
producing zones with different air densities \gccor{(compressions-rarefactions)}{, leading to the compressions-rarefactions phenomenon} 
\sidenote{Sound needs a medium to travel: it cannot travel through a vacuum.\\Unfortunately, there is no sound in outer space.}.
Such vibration of molecules takes place in the direction of the excitement, with the next layer of molecules excited by the \gccor{first layer}{previous one}.
Pushing layer by layer forward, a \textit{longitudinal \gc{mechanical} wave}\sidenote{%
...
} is \gccor{created}{generated}.

\gcrm{Under a this perspective,
\begin{center}
    \textit{sound is a longitudinal, mechanical wave}.
\end{center}}


\gccor{Such}{A} \textit{wave} \gcrm{is a disturbance that } propagates though a medium, which \gccor{could}{can} be solid, liquid or gaseous.
\marginpar{%
    \centering
    \footnotesize
...
the speed of sound can be \gccor{computed with the following approximated formula}{approximated as follows}:
\begin{equation}
...
The \gccor{changes in air pressure}{air pressure variations} at one point in space can be represented by a \textit{waveform}, which is a graphical representation of a sound
...
In general, the sound field is complex and can be decomposed as a superimposition of several waves~\citeonly{kuttruff2016room}.

We think of this process in the light of the classic \textit{source-medium-receiver} model of communication theory.
\begin{description}
    \item[source] \gcrm{is anything that} emits or expends energy (waves)\sidenote{%
    ...
    \item[medium] \gccor{is the vehicle for carrying}{carries} waves from one point to another, and
    \item[receiver] absorbs such waves.
\end{description}

\gccmt{Non mi piace l'elenco source medium reciver, scriverei nel testo}

\gcrm{The behavior of acoustic waves is defined by the acoustic-wave equation.
The rest of this section introduce of this equation, re-arranging the derivation presented in \citeonly{kuttruff2016room, pierce2019acoustics, marczuk2006modelling, avanzini2019sound}.
The derivation of the wave equation in light of room acoustic is presented in~\cref{ap:wave2rir}.}

\subsection{The acoustic wave equation}\label{subsec:acoustics:waveq}
\gccor{The behavior of acoustic waves is defined by the acoustic-wave equation,}{The acoustic wave equation is} 
a second-order partial differential equation\sidenote{%
...
The constant $\speedOfSound$ is the sound velocity in the medium \gccor{with}{and has} dimension $[\frac{\si{\metre}}{\si{\second}}]$.

Despite its complicated formulation, the wave equation is linear \gccor{. This has two implications}{leading to the following implications}~\citeonly{kuttruff2016room}:.

\begin{itemize}
    \item the pressure field at any time is the sum of the pressure fields resulting from each source at that time;
    \item the pressure field emitted \gccor{at}{from} a given source propagates over space and time according to a linear operation.
\end{itemize}
...
In general media are not uniform and feature inhomogeneities of two types:
...
In the presence of a sound source producing waves with distribution function $s(t, \positionMicrophone)$, the wave equation can be written\marginpar{%
    \footnotesize
    The minus sign is a conventions.
}
\begin{equation}
...
\textsc{The Green's Functions} are mathematical tools for solving linear differential equations with specified initial- and boundary- conditions~\citeonly{duffy2015green}.
...
\begin{center}
    ...
    \\ \emph{impulse responses}\sidenote{%
        Impulse responses in time domain, transfer fuctions in the frequency domain.}
...
There are two main categories: geometric and wave-based methods~\citeonly{habets2006room, savioja2015overview, thomas2017wayverb}.
\begin{description}
...
\\As stated in \citeonly{valimaki2016more}, among the wave-based methods, \DWMf/ are usually preferred:
...
such as probability distribution of the \RIR/ in regions of time-frequency domain~\citeonly{badeau2019common}.
...
\textsc{Ray-tracing}~\citeonly{kulowski1985algorithmic} is one the most common method that fall in this category and very popular
...
Many technique have been proposed to reduce the computational load, among all the \textit{diffuse rain algorithm}~\citeonly{schroder2007fast, heinz1993binaural}
...
The most popular is the Allen and Barkley's \ISMdef/~\citeonly{allen1979image}.
...
\\In the appendix of \citeonly{allen1979image}, the authors also proved that this method produce a solution the Helmholtz's equation
...
\begin{description}
    ...
    In the context of virtual reality, correct modeling of these quantities is essentials in order to maintain a coherent depth impression~\citeonly{kearney2012distance}.
\end{description}
...
\begin{center}
    \textit{The \emph{direct-to-reverberant ratio} ($\DRR$) quantifies the power of direct against indirect sound~\citeonly{zahorik2002direct}.}
\end{center}
