...
\subsection{the Image Source Model and the Image Source Method}

The \textit{image source model} simulates the reverberation in a room given source and receiver position.
It is based on purely specular reflection and it assumes that the sound energy travels around a scene in “rays”.

Based on this, Allen and Berkley developed an efficient method, the \ISMf/, to compute a \FIRf/ that model
the acoustic channel between the source and the receiver in rectangular rooms.
In the appendix of their They also proved that this method produce a close solution to the one the Helmholtz's equation.
...
\newthought{The Image Model} defines the interaction of the propagating sound with the reflectors.
