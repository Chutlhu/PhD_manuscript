\chapter{Elements of Room Acoustics}\label{ch:acoustics}
...
\mynewline
The material on waves and acoustic reflection is digested from classical texts on room acoustics~\citeonly{kuttruff2016room,pierce2019acoustics} and on partial differential equations~\citeonly{duffy2015green}.
...
\marginpar{%
    ...
    \captionof{figure}{%
    \itshape
    Imagine a calm pond. The surface is flat and smooth. Drop a rock into it.
    \textit{Kerploop!} The surface is now disturbed.
    The disturbances spread and propagate, as waves. The medium here is the water surface.
    }
...
\\Thus, sound propagates through a medium, which can be solid, liquid or gaseous.
...
\begin{figure}[h]
    ...
    \sideparmargin{outer}
    \sidepar{\vspace{-10\baselineskip}
        \captionof{figure}{%
            \footnotesize
            Illustration of the molecules under sound compression and rarefaction due to longitudinal sound wave and its waveform representation.
        }
...
the \textit{source} is anything that emits waves\sidenote{%
    ...
    or air vortices (\eg/ flute and whistles).
}, the \textit{medium} carries the waves from one point to another, and the \textit{receiver} receives them.
...
Equation~\ref{eq:acoustics:wave} is expressed in the space-time domain $\depSpaceTime$.
...
\begin{figure}[t]
    \begin{fullwidth}
        ...
        \captionof{figure}{%
            \footnotesize
            Different types of sound interaction with a surface.
        }
...
\marginpar{
    ...
    \captionof{figure}{%
        \itshape
        Schlieren photographs showing successive stages in the progress of a sound pulse in a section of a Debating Chamber.
        \imgsrc{\citeonly{davis1926sound}.
        \textit{Sabine used Schlieren photography to investigate sound propagation paths in the early 1900s.
        Their impressive visualizations show wavefronts that are augmented with rays that are perpendicular to the wavefronts}}
    }
    \label{fig:acoustics:Schlieren}
}
...
\newthought{Echoes are specular reflections}\marginpar{%
    ...
    In the folk stories of Greece, Echo is a mountain nymph whose ability to speak was cursed:
    she was only able to repeat the last words anyone spoke to her.
...
% -----------------------------------------------------------------------------
\section{Room acoustics and room impulse responses}\label{ch:acoustics:sec:rir}
...
\begin{figure}[t]
    ...
    \end{minipage}
    \caption{Schematic illustration of the shape of an \ac{RIR} and the first 100 ms of a measured one. Image taken from~\citeonly{vincent2018audio}}
...
\marginpar{
    \footnotesize
    The docoumentation of the \href{https://reuk.github.io/wayverb/introduction.html}{Wayverb\ExternalLink}
...
\newthought{Wave-based Methods} are iterative methods that divide the 3D bounded enclosure into a grid of interconnected nodes\sidenote{\ie/ mechanical unit with simple degrees of freedoms, like mass-spring system or one-sample-delay unit}.
...
\marginpar{
    ...
    \label{fig:acoustics:massspring}
}
...
less dense grids may simplify too much the simulation, while denser grids increase the computational load and other intrinsic limitations.\sidenote{
    For more details on this, the reader can refers to the \textit{Courant–Friedrichs–Lewy conditions} and the documentation of the
    \href{https://reuk.github.io/wayverb/introduction.html}{Wayverb\ExternalLink}.
}
...
In particular, by simulating accurately low-frequency components of the \ac{RIR}, they are able to well characterize the \textit{room modes}\sidenote{
    Room modes have the effect of amplifying and attenuating specific frequencies in the \ac{RIR},
    ...
    such as concert halls and recording studios or when producing musically pleasing reverbs.
...
such as probability distribution of the \ac{RIR} in regions of the time-frequency domain~\citeonly{badeau2019common}.
...
namely a \textit{directional-time-frequency energy map} of the room’s diffuse sound field for a given receiver location (see~\cref{fig:acoustics:diffuse_map}).\marginpar{
    \centering
    \footnotesize
    ...
    \captionof{figure}{%
        ...
        Image taken from~\citeonly{schimmel2009fast}
    }
    \label{fig:acoustics:diffuse_map}
} This map is then used as a prior distribution for drawing random sets of impulses which are used to form the \ac{RIR}.
...
\begin{figure}[t]
    \begin{fullwidth}
        ...
        \resizebox{\linewidth}{!}{
            ...
            \captionof{figure}{%
            \footnotesize
            Time-Frequency regions of the \ac{RIR} associated to the method that better simulate them.
            Image adapted from~\citeonly{thomas2017wayverb,badeau2019common}.
            }
...
\end{itemize}
Providing these time- and frequency-domain crossover points~\citeonly{badeau2019common} it is possible to combine image-source, ray-tracing and wave models.
...
Its application to acoustics was originally proposed by \citeauthor{allen1979image} in \citeonly{allen1979image} and it is know as the \ISMdef/.
...
In the appendix of \citeonly{allen1979image}, the authors also proved that this method produces a solution of the Helmholtz's equation
...
\begin{figure}[h]
    ...
    \captionof{figure}{%
        \footnotesize
        3-D representation of the \ISMdef/ and its propagation paths for selected echoes.
        }
...
\begin{figure}[h]
    ...
    \captionof{figure}{%
        \footnotesize
        From left to right, path involving the direct path, one reflection obtained using first-order image, and two reflections obtained using two images.
        Image inspired from~\citeonly{habets2006room}.
        }
        \label{fig:acoustics:2D_ims}
    }
\end{figure}
...
Therefore, in the time domain, the \ac{RIR} associated to the source at position $\positionSource$ and the receiver at $\positionMicrophone$ reads
...
\end{equation}
where $\positionSource_\idxEch$ is the $\idxEch$-th image of the source and $\bar{\absCoeff}_{\idxEch}$ is the total frequency-independent\sidenote{Which is equivalent to consider perfectly rigid and reflective walls} damping coefficient related to the $\idxEch$-th image.
...
\newthought{The early reflections and echoes} are reflections which are by nature highly correlated with the direct sound.
They convey a sense of geometry which modifies the general perception of the sound:
\begin{itemize}
    \item \textit{The precedence effect} occurs when two correlated sounds are perceived as a single auditory event~\citeonly{wallach1973precedence}.
    ...
    \item \textit{Apparent source width} is the audible impression of a spatially extended sound source~\citeonly{griesinger1997psychoacoustics}.
    ...
    In the context of virtual reality, correctly modeling these quantities is essential in order to maintain a coherent depth impression~\citeonly{kearney2012distance}.
\end{itemize}

\newthought{The late reverberation} is indicative of the size of the environment and the materials within~\citeonly{valimaki2016more}.
It produces the so-called the \textit{listener envelopment}, \ie/ the degree of immersion in the sound field~\citeonly{griesinger1997psychoacoustics}.
This portion of the \ac{RIR} is mainly characterized by sound diffusion, which depends on the surfaces roughness.

\subsection{Mixing time}
Perceptually, the mixing time could be regarded as the moment when the diffuse tail cannot be distinguished from that of any other position or listener’s orientation in the room~\citeonly{lindau2012perceptual}.
...
Due to this, it is an important parameter also in the context of \acp{RIR} synthesis as it defines cross-over point for room acoustics simulator using hybrid methods~\citeonly{savioja2015overview}(see \cref{sec:acoustics:simulators}).
...
The \acf{DRR} quantifies the power of direct against indirect sound~\citeonly{zahorik2002direct}.
