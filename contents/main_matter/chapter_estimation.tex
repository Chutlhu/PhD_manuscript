...
In general, due to the measurement process, such peaks are not necessarily positive, thus, to better visualize them, the \textit{echogram}~\citeonly{kuttruff2016room}, $\magnitudeOf{\tilde{\rir}(t)}$, or the energy envelop~\citeonly{schroeder1979integrated} are used instead\sidenote{
...
By modeling the \DOAs/, such approaches are able to implicitly reduces the ambiguity of the estimated echoes.
This idea is rooted in existing methods used in multipath communication system, denoted as \JADE/~\citeonly{vanderveen1997joint,verhaevert2004direction}.
