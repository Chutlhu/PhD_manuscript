...
\\The \acp{ReIR} (and their frequency counterparts, \acp{ReTF}) have been introduced with the explicit purpose of avoiding the computation of the acoustic channel related to each microphone~\citeonly{gannot2001signal}.
...
Compared to the difficult task of estimating the acoustic channels and of relying on bad channel estimates, this is typically sufficient for achieving good enhancement performances in many practical scenarios.\sidenote{
    Note that, as opposed to channel estimation, estimating the \acp{ReTF} is a non-blind problem (See~\cref{sec:processing:rirmodels})}
Since then, \acp{ReIR} have been incorporated in powerful beamforming algorithms, used for both dereverberation and noise reduction (\eg/ \citeonly{schwartz2014multi, kodrasi2017evd}).
...
Later, this approach has been extended to enhance desired signals in the context of the \textit{cocktail party problem} in~\citeonly{dokmanic2015raking} and for noise and reverberation reduction~\citeonly{peled2013linearly, Javed2016spherical, Kowalczyk2019raking}.
In particular, the work~\citeonly{peled2013linearly} estimates early echoes in the spherical harmonic domain and uses their \acp{DOA} to build \ac{ReIR}.
...
the first early echoes are shown to contribute to increasing speech intelligibility, as they are fully integrated into the direct sound, increasing its perceived intensity~\citeonly{bradley2003importance}.
...
\newthought{The source} covariance matrix $\xpsd_\src$ is assumed here to be diagonal, since we assume that all the spatial information is expressed by the filters $\FLTSS^{\mathtt{ec}}$ and the source signals are independent to each other, that is,
\begin{equation}
...
\newthought{The late reverberation} covariance matrix can be estimated using the time-invariant spatial coherent matrix model proposed in~\citeonly{kuster2012objective}, based a diffuse sound field model~\citeonly{kuttruff2016room}.
...
where $\sigma_\mathtt{lr}^2$ denotes the power of the late reverberation and $\mathbf{\Gamma}$ is the $\numMics \times \numMics$ spatial coherence matrix, which is available in closed-form.\sidenote{
    ...
    \begin{equation*}
        \tilde{\gamma}_{ii'}[k] = \frac{\sin(2 \pi f_k \distMicMic_{ii'} / \speedOfSound)}
                                        {2 \pi f_k \distMicMic_{ii'} / \speedOfSound}.
    \end{equation*}
    Then, the matrix $\mathbf{\Gamma} = \kbracket{\tilde{\gamma}_{_{ii'}}}$ is built for all the pairs of channel on a discrete set of frequencies.
}
...
\newthought{The additive noise} term $\NSES$ is assumed to be stationary over the recording.
Therefore its covariance matrix can be estimated from the observation using non-speech segments.
In a blind setting, this would require the usage of a voice activity detector.
Alternatively, the noise component can be modeled as a stationary random process whose spatial covariance matrix can be estimated with advanced techniques exploiting speech non-stationarity~\citeonly{gannot2001signal}.
...
\begin{equation}
    \bsD' = \klist{
                1,
                \cste^{-\csti 2 \pi f_k \tau_{\idxMic}^{(0)} / \speedOfSound},
                \ldots,
...
\newthought{The \MVDRtxt/} beamformer optimizes the following criterion\sidenote{
    ...
    \begin{equation*}\scriptsize
        \bsW_{\mathtt{MPDR}} = \kargmin_{\bsW} \khermitian{\bsW} \xpsd_x \bsW \;\text{s.t.}\;\khermitian{\bsW} \FLTS = 1.
    \end{equation*}
...
\begin{equation}
    \bsW_{\MVDR} = \kargmin_{\bsW} \khermitian{\bsW} \xpsd_n \bsW \;\text{s.t.}\;\khermitian{\bsW} \FLTS = 1
    .
\end{equation}
...
Therefore, the reduction of the output energy suppresses any external noise modeled by $\xpsd_{n}$.
...
\begin{equation}\label{eq:dechorateapp:mvdr}
    \bsW_{\MVDR} = \frac{\kinv{\xpsd}_{n} \FLTS}
                                {\khermitian{\FLTS} \kinv{\xpsd}_{n} \FLTS}
    .
\end{equation}
...
This criterion design can be easily extended to work with any type of noise and acoustic transfer functions modeled by $\xpsd_{n}$ and $\FLTS$, respectively.
...
They can be built on the knowledge of the \TOAs/ of the source signal and their integration in the \ac{MVDR} criterion is straightforward:
...
Moreover, knowing the microphone array geometry, the \TDOAs/ may be derived from the source's \DOA/\sidenote{
    The mapping between \TDOAs/ and \acp{DOA} is not always unique. It depends on the microphone array geometry, such as its compactness, its shape, and the number of sensors.
}, thus, the steering vectors can be easily computed. However, \acp{DOA} need to be estimate aside, using \acf{SSL} methods (See~\cref{sec:application:localization}).
...
Given the \RTF/ of the $\idxMic$-th channel $\FLT_\idxMic$, its \acp{ReTF} with respect to the first channel is given by
\begin{equation}
...
\newthought{The noise contribution} is taken into account through the covariance matrix $\xpsd_n$.
...
As long as the power spectral density of the modelled noise source is available, its suppression can be achieved by including it in $\xpsd_n$ and using it in~\cref{eq:dechorateapp:mvdr}.\sidenote{
...
If these excerpts are long enough, the whole covariance matrix $\xpsd_n$, including both noise and interferer, is well approximated by its sampled version whose calculation is straightforward.
...
\begin{itemize}
    ...
    \item the \acp{ReTF}, $\FLTS_{\mathtt{ReTF}}$, is estimated from the observed signal using the \acf{GEVD} method described in~\citeonly{doclo2003robust};
...
Both real and corresponding synthetic \RIRs/ are used, which are then convolved with anechoic utterances from the \WSJ/ corpus and corrupted by recorded diffuse noise.
The correspondence between real and synthetic \RIRs/ is a property of the \dEchorate{} dataset and is described in~\cref{ch:dechorate}.
...
\mynewline
The evaluation is conducted similarly to the one in~\citeonly{dokmanic2015raking,kodrasi2017evd,Kowalczyk2019raking}.
...
namely, $\MICS^{\mathtt{ec}}$ in~\cref{eq:dechorateapp:target}.
On one hand, this choice numerically penalizes both direct-path-based and \ReTF/-based beamformers which aim at extract the direct-path signal only and the reverberant signal at the reference microphone, respectively.
On the other hand, considering only the direct path or the full reverberant signal would be equally unfair for the other beamformers.
Moreover, choosing the early contribution as target signal is perceptually motivated by the fact that early reflection contributes to speech intelligibility~\citeonly{bradley2003importance}.
...
Denoting with $\MIC_1$ the signal at the reference microphone and $\MIC^{\mathtt{ec}}_{1}$ the target speech at the same microphone (See~\cref{eq:dechorateapp:target}), these quantities are defined as follows:
\newcommand{\hbsW}{\khermitian{\bsW}}
    \begin{equation}
        \begin{aligned}
            \mathtt{SNRR}_\mathtt{i} &= 10 \log_{10} \kparen{\frac{\sigma^2_{\MIC^{\mathtt{ec}}_{ 1}}}
                                                            {\sigma^2_{\MIC_{1}} -  \sigma^2_{\MIC^{\mathtt{ec}}_{ 1}}}} \quad\text{[dB]}\\
            \mathtt{SNRR}_\mathtt{o} &= 10 \log_{10} \kparen{\frac{\sigma^2_{\hbsW \MICS^{\mathtt{ec}}}}
                                                            {\sigma^2_{\hbsW \MICS} -  \sigma^2_{\hbsW \MICS^{\mathtt{ec}}}}} \quad\text{[dB]}\\
...
From the loudness spectra differences, a Mean Opinion Score (MOS)\sidenote{
    ...
    MOS were originally obtained from perceptual tests involving human subjects.
...
\begin{figure}[t]
    \begin{fullwidth}
        \centering
        \includegraphics[trim={0 10 10 0},clip,width=\linewidth]{figures/dechorateapp/kowalkzy_results_boxplot1.pdf}
        \caption{
...
\begin{enumerate}
    ...
    \item estimation of the image source position either through  \textit{multilateration}~\citeonly{dokmanic2013acoustic, dokmanic2015relax} (see below), \MLdef/~\citeonly{tervo2011localization} or convex optimization~\citeonly{crocco2012closed};
...
Finally, the 2 outliers for source \#3 are due to a wrong annotation caused by the source directivity and misclassification.
...
\mynewline
Future works will explore several directions. Regarding the applications, the echo-aware methods presented above could be validated over more challenging scenarios than the one presented.
...
By the amount of the data collected, they can be used to train learning-based signal processing methods or for data augmentation.
...
Moreover, by using the pairs of simulated \vs/ real \RIRs/ available in the dataset, it could be possible to develop techniques to convert one to the other, using style transfer and domain adaptation techniques.
\qed
